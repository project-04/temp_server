class scoreboard extends uvm_scoreboard;
	...
	uvm_tlm_analysis_fifo #(read_xtn) dest_fifo_0;
	uvm_tlm_analysis_fifo #(read_xtn) dest_fifo_1;
	uvm_tlm_analysis_fifo #(read_xtn) dest_fifo_2;
	write_xtn wr_xtn;
	read_xtn rd_xtn0,rd_xtn1,rd_xtn2;
	bit [7:0] src_array[$],dest_array[$];
	static int matched     = 0;
	static int mis_matched = 0;
	
	function new(string name = "scoreboard",uvm_component parent = null);
		super.new(name,parent);
		src_fifo    = new("src_fifo");
		dest_fifo_0 = new("dest_fifo_0");
		dest_fifo_1 = new("dest_fifo_1");
		dest_fifo_2 = new("dest_fifo_2");
	endfunction 
	...
	fork
		forever
			begin
				src_fifo.get(wr_xtn);
				`uvm_info("SCOREBOARD", $sformatf(" SRC_MON TO SB \n%s", wr_xtn.sprint()), UVM_LOW)
				write_array(wr_xtn);
			end
		forever
			begin
				dest_fifo_0.get(rd_xtn0);
				if(rd_xtn0!=null)
					begin
						`uvm_info("SCOREBOARD", $sformatf(" DEST_MON_0 TO SB \n%s", wr_xtn.sprint()), UVM_LOW)
						compare_pkt(rd_xtn0);
					end
		...
		forever
			begin
				dest_fifo_1.get(rd_xtn1);
				if(rd_xtn1!=null)
					begin
						`uvm_info("SCOREBOARD", $sformatf(" DEST_MON_1 TO SB \n%s", wr_xtn.sprint()), UVM_LOW)
						compare_pkt(rd_xtn1);
					end
			end
		forever
			begin
				dest_fifo_2.get(rd_xtn2);
				if(rd_xtn2!=null)
					begin
						`uvm_info("SCOREBOARD", $sformatf(" DEST_MON_2 TO SB \n%s", wr_xtn.sprint()), UVM_LOW)
						compare_pkt(rd_xtn2);
					end
			end
	join
	...
	task write_array(write_xtn wr_xtn);
		src_array.push_back(wr_xtn.header);
		
		for(int i = 0; i<wr_xtn.header[7:2] ; i++)
			begin
			//foreach(wr_xtn.payload[i])
				src_array.push_back(wr_xtn.payload[i]);
			end
		//#1;
		src_array.push_back(wr_xtn.parity);
		$display("SRC REF ARRAY");
		foreach(src_array[i])
			$write("%0d,", src_array[i]);
		$display("");
	endtask
	
	task compare_pkt (read_xtn pkt);
		dest_array.push_back(pkt.header);
		
		for(int i = 0; i<pkt.header[7:2] ; i++)
			begin
				//foreach(pkt.payload[i])
					dest_array.push_back(pkt.payload[i]);
			end
		//#1;
		dest_array.push_back(pkt.parity);
		
		$display("DEST REF ARRAY");
		foreach(dest_array[i])
			$write("%0d,", dest_array[i]);
		
		if(src_array[0:$]==dest_array[0:$])
			begin 
				matched++;
				$display("\nsrc data is equal to dest data - %0d",matched);
				src_array.delete();
				dest_array.delete();
				$display("");
			end
			...
			begin
				mis_matched++;
				$display("\nsrc data is not equal to dest data - %0d",mis_matched);
				src_array.delete();
				dest_array.delete();
				$display("");
			end
    ...
    function void report_phase(uvm_phase phase);
		...
		$display("");
        `uvm_info("SCOREBOARD", $sformatf("\nFinal Score - Matched: %0d, Mismatched: %0d", 
                 matched, mis_matched), UVM_NONE)
    endfunction
