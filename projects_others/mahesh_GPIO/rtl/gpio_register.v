...
///////////////////////// rgpio_oe /////////////////////////////////
	always @(posedge sys_clk or posedge sys_rst) begin
		if(sys_rst)
			...
			rgpio_oe <= ((gpio_addr == RGPIO_OE) && gpio_we) ? gpio_dat_i[31:0]: rgpio_oe;
	end	
...
//////////////////////////////// rgpio_ctrl /////////////////////////
	always @(posedge sys_clk or posedge sys_rst) begin
		if(sys_rst)
			...
			rgpio_ctrl <= ((gpio_addr == RGPIO_CTRL) && gpio_we) ? gpio_dat_i[1:0] : ((rgpio_ctrl[RGPIO_CTRL_INTE]) ? {rgpio_ctrl[RGPIO_CTRL_INTS]|gpio_inta_o,rgpio_ctrl[RGPIO_CTRL_INTE]}:rgpio_ctrl);
	end
...
//////////////////////////////////// rgpio_ints/////////////////////
	always @(posedge sys_clk or posedge sys_rst) begin
		if(sys_rst)
			...
			if((gpio_addr == RGPIO_INTS) && gpio_we)
				rgpio_ints <= gpio_dat_i[31:0];
			else 
				rgpio_ints <= (rgpio_ctrl[RGPIO_CTRL_INTE]) ? (rgpio_ints |(rgpio_inte & (rgpio_in ^ in_mux) & (~(rgpio_ptrig ^ in_mux)))): rgpio_ints;
		end
	end
