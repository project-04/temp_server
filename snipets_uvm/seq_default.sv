module seq_default;
		...
		task run_phase(uvm_phase phase);
			forever
			       begin
				    ...
				    `uvm_info("DRIVER", $sformatf("From drv%s",req.sprint()), UVM_LOW)
				    #1;
	...
	class A_equals_to_7_seq extends seq;
		...
		task body();
			if(starting_phase!=null) starting_phase.raise_objection(this, "starting my_seq");
	...
	class test_A_7 extends test;
		...
		function void build_phase(uvm_phase phase);
			...
			//A_7_seq = A_equals_to_7_seq::type_id ::create("A_7_seq");
	 	     	//uvm_config_db #(uvm_sequence_base)::set(this, "envh.agth[0].seqrh.run_phase", "default_sequence", A_7_seq);
	 	     	
	 	     	uvm_config_db #(uvm_object_wrapper)::set(this, "envh.agth[0].seqrh.main_phase", "default_sequence", A_equals_to_3_seq::type_id::get());
		endfunction
	...
	initial begin
		//qverilog seq_default.sv
		//vcs -sverilog -ntb_opts uvm seq_default.sv; ./simv
...
/*
Start time: 09:45:58 on Jun 29,2026
qverilog ../seq_default.sv 
-- Compiling module seq_default
** Note: (qverilog-2286) ../seq_default.sv(3): Using implicit +incdir+/home/cad/eda/Mentor_Graphics/Questasim/questasim/uvm-1.1d/../verilog_src/uvm-1.1d/src from import uvm_pkg
-- Importing package mtiUvm.uvm_pkg (uvm-1.1d Built-in)

Top level modules:
	seq_default
# vsim -lib work work.seq_default -c -do "run -all; quit -f" -appendlog -l qverilog.log -vopt 
...
# --------------------------------------------------------------
# 
# UVM_INFO ../seq_default.sv(54) @ 0: uvm_test_top.envh.agth[0].drvh [DRIVER] From drv------------------------------------------------------------------------------------------------
# Name                           Type      Size  Value                                            
# ------------------------------------------------------------------------------------------------
# req                            xtn       -     @673                                             
#   begin_time                   time      64    0                                                
#   depth                        int       32    'd2                                              
#   parent sequence (name)       string    17    A_equals_to_3_seq                                
#   parent sequence (full name)  string    49    uvm_test_top.envh.agth[0].seqrh.A_equals_to_3_seq
#   sequencer                    string    31    uvm_test_top.envh.agth[0].seqrh                  
#   A                            integral  4     'd3                                              
# ------------------------------------------------------------------------------------------------
# 
# UVM_INFO ../seq_default.sv(54) @ 1: uvm_test_top.envh.agth[0].drvh [DRIVER] From drv------------------------------------------------------------------------------------------------
# Name                           Type      Size  Value                                            
# ------------------------------------------------------------------------------------------------
# req                            xtn       -     @678                                             
...
# ------------------------------------------------------------------------------------------------
# 
# UVM_INFO ../seq_default.sv(54) @ 2: uvm_test_top.envh.agth[0].drvh [DRIVER] From drv------------------------------------------------------------------------------------------------
# Name                           Type      Size  Value                                            
# ------------------------------------------------------------------------------------------------
# req                            xtn       -     @682                                             
...
#    Time: 3 ns  Iteration: 106  Instance: /seq_default
# End time: 09:46:02 on Jun 29,2026, Elapsed time: 0:00:04
# Errors: 0, Warnings: 0
*/
