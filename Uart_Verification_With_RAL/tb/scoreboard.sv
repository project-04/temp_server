...
//      if((thr1size==0)&&(thr2size==0)) $display("NO DATA IN THR1 AND THR2. SOMETHING WENT WRONG. \n");
        ...
        task ral;
                ...
                `uvm_info("REG",$sformatf("@@@@@@@@@@@@@@@@ LCR %d" ,reg_lcr),UVM_NONE)

        //      regmodel.ier.read(status,reg_lcr,UVM_BACKDOOR, .map(regmodel.default_map));
        //      `uvm_info("REG",$sformatf("@@@@@@@@@@@@@@@@ IER %d" ,reg_ier),UVM_NONE)

                if(uart1.LCR == reg_lcr)
                `uvm_info("REG_COMP", "#################### LCR", UVM_LOW)

        //      if(uart1.ier == reg_ier)
        //      `uvm_info("REG_COMP", "#################### IER", UVM_LOW)
