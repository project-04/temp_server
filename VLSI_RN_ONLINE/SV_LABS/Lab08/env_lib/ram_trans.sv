...
class ram_trans;
   ...
   constraint C0 {(trans_id <  5) -> (write == 1'b1 && read == 1'b0 && rd_address == 12'd0 && wr_address inside{[0:4]});
   	  (trans_id >= 5) -> (write == '0 && read == '1 && wr_address == '0 && rd_address inside{[0:4]});}
          
          
   //constraint C0 {
    //(trans_id < 5) -> (write == 1 && read == 0 && wr_address inside {[0:4095]} && rd_address == 0);
    //(trans_id >= 5) -> (write == 0 && read == 1 && wr_address == 0 && rd_address inside {[0:4095]});}
	...
	*/
	...
	//if(trans_id < 5)              begin write = 1'b1; read = 1'b0; wr_address = temp_addr; rd_address = 12'd0; temp_addr = temp_addr+1'b1; end
