...
function void ram_base_test::config_ram();
    if (has_wagent)
		begin 
			...
			if(!uvm_config_db #(virtual ram_if)::get(this, "", "ram_if", m_wr_cfg.vif))
        `uvm_fatal("RAM_TEST", "cant get the virtual ram_if");
    ...
    if(has_ragent) 
      ...
      m_rd_cfg.is_active = UVM_ACTIVE;
			...
			// Get the virtual interface from the config database "vif"
			if(!uvm_config_db #(virtual ram_if)::get(this, "", "ram_if", m_rd_cfg.vif))
        `uvm_fatal("RAM_TEST", "cant get the virtual vif");
...
class ram_single_addr_test extends ram_base_test;
   ...
   // Factory Registration
	...
	// Declare the handle for  ram_single_vseq virtual sequence
 	ram_single_vseq ram_single_vseqh;
	//------------------------------------------
	// METHODS
	//------------------------------------------
...
task ram_single_addr_test::run_phase(uvm_phase phase);
	...
	//create instance for sequence
   	ram_single_vseqh = ram_single_vseq::type_id::create("ram_single_vseqh");
	//start the sequence wrt virtual sequencer
   	ram_single_vseqh.start(ram_envh.v_sequencer);
	//drop objection
...
task ram_ten_addr_test::run_phase(uvm_phase phase);
	//raise objection
  	phase.raise_objection(this);
	//create instance for sequence
   	ram_ten_vseqh = ram_ten_vseq::type_id::create("ram_ten_vseqh");
	//start the sequence wrt virtual sequencer
   	ram_ten_vseqh.start(ram_envh.v_sequencer);
	//drop objection
	phase.drop_objection(this);
endtask   
...
task ram_odd_addr_test::run_phase(uvm_phase phase);
	//raise objection
  	phase.raise_objection(this);
	//create instance for sequence
   	ram_odd_vseqh = ram_odd_vseq::type_id::create("ram_odd_vseqh");
	//start the sequence wrt virtual sequencer
   	ram_odd_vseqh.start(ram_envh.v_sequencer);
	//drop objection
	phase.drop_objection(this);
endtask   
...
task ram_even_addr_test::run_phase(uvm_phase phase);
	//raise objection
  	phase.raise_objection(this);
	//create instance for sequence
   	ram_even_vseqh = ram_even_vseq::type_id::create("ram_even_vseqh");
	//start the sequence wrt virtual sequencer
   	ram_even_vseqh.start(ram_envh.v_sequencer);
	//drop objection
	phase.drop_objection(this);
endtask   
