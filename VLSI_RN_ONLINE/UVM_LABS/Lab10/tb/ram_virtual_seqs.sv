...
// get the config object ram_env_config from database using uvm_config_db 
	 	if(!uvm_config_db#(ram_env_config)::get(null,get_full_name(),"ram_env_config",m_tb_cfg))
		`uvm_fatal("CONFIG","cannot get() the config. have you set it?")

// initialize the dynamic arrays for write & read sequencers to m_tb_cfg.no_of_duts
		wr_seqrh = new[m_tb_cfg.no_of_duts];
		rd_seqrh = new[m_tb_cfg.no_of_duts];
...
//-----------------  task body() method  -------------------//
		...
		task ram_single_vseq::body();
		 ...
		 single_rxtns = ram_single_addr_rd_xtns::type_id::create("single_rxtns");
                    ...
                    if(m_tb_cfg.has_wagent) begin
                   for (int i=0 ; i < m_tb_cfg.no_of_duts; i++)
                  // LAB : Start the write sequence on all the write sequencers 
	              single_wxtns.start(wr_seqrh[i]);
                   end

                   if(m_tb_cfg.has_ragent) begin
                   for (int i=0 ; i < m_tb_cfg.no_of_duts; i++)
