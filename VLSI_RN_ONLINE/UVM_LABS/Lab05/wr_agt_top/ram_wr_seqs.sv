  ...
  repeat(10)
    begin
      //write_xtn req;
  		req = write_xtn::type_id::create("req");
		  start_item(req);
		  assert(req.randomize() with {address inside{[0:9]};});
		  finish_item(req);
    end
endtask
  ...
  repeat(10)
    begin
      //write_xtn req;
  		req = write_xtn::type_id::create("req");
		  start_item(req);
		  assert(req.randomize() with {address % 2 == 1;});
		  finish_item(req);
    end
endtask
...
task ram_even_wr_xtns::body();
  repeat(10)
    begin
      write_xtn request_testing;
  		request_testing = write_xtn::type_id::create("req");
		  start_item(request_testing);
		  assert(request_testing.randomize() with {address % 2 == 0;});
		  finish_item(request_testing);
    end
endtask
