...
function void ram_tb::build_phase(uvm_phase phase);
	...
	if(m_cfg.has_wagent) begin
		if(!uvm_config_db #(ram_wr_agent_config)::get(this,"","ram_wr_agent_config", m_cfg.m_wr_cfg))
			`uvm_fatal("CONFIG","cannot get() m_cfg.m_wr_cfg from uvm_config_db. Have you set() it?")
	...
	if(m_cfg.has_ragent) begin
		if(!uvm_config_db #(ram_rd_agent_config)::get(this,"","ram_rd_agent_config", m_cfg.m_rd_cfg))
			`uvm_fatal("CONFIG","cannot get() m_cfg.m_rd_cfg from uvm_config_db. Have you set() it?")
