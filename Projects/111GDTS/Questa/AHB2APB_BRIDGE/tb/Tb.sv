...
endclass
	...
	/*	if(m_cfg.has_apb_agent) begin
			foreach(v_sequencer.apb_sqrh[i]) begin
				v_sequencer.apb_sqrh[i] = apb_top.agnt[i].seqr_h;
			end
		end */
		if(m_cfg.has_scoreboard) begin
			...
			foreach(sb.fifo_apb_h[i]) begin
				apb_top.hagnt[i].hmon.monitor_port.connect(sb.fifo_apb_h[i].analysis_export);
			end
		end 
	endfunction
