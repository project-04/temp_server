...
//----------------------- INCR UNSPECIFIED VIRTUAL SEQUENCE ------------------------//

class ahb_unspec_vseq extends base_virtual_seqs;

	`uvm_object_utils(ahb_unspec_vseq)
	
	unspecified_incr_wr_seq unspec_seq;
	
	extern function new(string name = "ahb_unspec_vseq");
	extern task body;
	
endclass

function ahb_unspec_vseq::new(string name = "ahb_unspec_vseq");
	super.new(name);
endfunction

task ahb_unspec_vseq::body;
	 super.body();
     
        if(e_cfg.has_ahb_agent)  unspec_seq = unspecified_incr_wr_seq::type_id::create("unspec_seq");

        unspec_seq.start(ahb_seqrh[0]);
endtask

//----------------------- AHB WRAP VIRTUAL SEQUENCE ------------------------//

class ahb_wrap_vseq extends base_virtual_seqs;

	`uvm_object_utils(ahb_wrap_vseq)
	
	ahb_wrap_wr_seq wrap_seq;
...
task ahb_wrap_vseq::body;
	 ...
	 wrap_seq = ahb_wrap_wr_seq::type_id::create("wrap_seq");

        wrap_seq.start(ahb_seqrh[0]);
endtask
