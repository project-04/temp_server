		...
		begin
		  if(txn.hsize==0)
			...
			if(txn.haddr[1:0]==2'b00)
			     ...
			     txn.master_read_mem[txn.haddr]=vif.mmcb.hrdata[31:0];
		
		 $display($time, "master monitor read operation mem[%0d]=%0d",txn.haddr,txn.master_write_mem[txn.haddr]);
		txn.hresp=vif.mmcb.hresp;
		end

		`uvm_info("master monitor",$sformatf("Printing from master monitor \n %s",txn.sprint),UVM_LOW)
		if(txn.hwrite)
		 $display($time, "master monitor write operation mem[%0d]=%h",txn.haddr,txn.master_write_mem[txn.haddr]);
		else
		 $display("master monitor read operation mem[%0d]=%h",txn.haddr,txn.master_write_mem[txn.haddr]);
