...
io_cfg m_cfg;
	...
	extern task send_to_dut(io_xtn xtn);
endclass
...
task io_driver::send_to_dut(io_xtn xtn);

	@(vif.io_pad_drv_cb);
	vif.io_pad_drv_cb.io_pad <= xtn.io_pad;

	`uvm_info("io_pad__driver",$sformatf("Printing from IO_PAD Driver class %s \n",xtn.sprint),UVM_LOW)
endtask
