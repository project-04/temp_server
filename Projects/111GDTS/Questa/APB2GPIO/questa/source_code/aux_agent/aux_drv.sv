...
aux_cfg m_cfg;
	...
	extern task send_to_dut(aux_xtn xtn);
endclass
...
task aux_driver::send_to_dut(aux_xtn xtn);

	@(vif.aux_input_drv_cb)
	vif.aux_input_drv_cb.aux_in <= xtn.aux_in;

	`uvm_info("aux_driver",$sformatf("Printing from AUX_IN Driver class %s \n",xtn.sprint),UVM_LOW)
endtask
