		...
		begin
		  if(txn.hsize==0)
			...
			if(txn.haddr[1:0]==2'b00)
			     ...
			     txn.slave_read_mem[txn.haddr]=vif.smcb.hrdata[31:0];
		
		end
		if(txn.hwrite)	
		 $display($time, "slave monitor write operation mem[%0d]= %h",txn.haddr,txn.slave_write_mem[txn.haddr]);
	        else
		$display($time, "slave monitor read operation mem[%0d]= %h",txn.haddr,txn.slave_write_mem[txn.haddr]);
