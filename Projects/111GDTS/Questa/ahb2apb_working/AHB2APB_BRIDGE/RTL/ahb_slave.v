   ...
   `include "definitions.v"
     module ahb (
	  ...
	  reg   [1:0] pstate,nstate,pre_state,next_state;
	  ...
	  reg   [2:0] Hburst_reg, Hsize_reg, Hburst_reg_2, Hsize_reg_2;
	...
	assign  valid           =  (Hresetn & Hreadyin & Hresp!=2'b10 & Htrans !=2'b01 & Htrans!=2'b00 & 
                                   (!(Haddr >=0 & Haddr <=8) & Hwrite == 1  )? 1'b1  : 1'b0;
	assign  Hresp 	        =  (Hresp_flag) ? 2'b10 :  0;
	...
	begin:BLOCK6
 	  if(~Hresetn)
 	    ...
 	    begin:BLOCK5
     	     error <= 0;
              if(pre_state == OKAY & Htrans == 2'b11)
	        begin:BLOCK4
		 if(!(Haddr >=0 && Haddr <=8)) 
		  begin :BLOCK1
		    error <= 0;
		...
		3'b111 : begin
		  ...
		  begin:BLOCK3
	 	    if((Hsize != Hsize_reg) || (Hburst != Hburst_reg) || (Hwrite != Hwrite_reg))
		      begin
     ...
     always @(pre_state,error)
      begin
	   ...
	   ERR1 : begin
		   $display("inside ERR1 state", $time);
		   next_state = ERR2;
		   Hresp_flag_im = 2'b10;
		   Hresp_Hreadyout_im = 0;
		 end
	...
	always @( pstate, Hresetn, Hreadyin, Htrans, Hwrite, Hwdata, Hburst, Hsize, Haddr, Hreadyout)
	begin
	   case(pstate)
	     IDLE : 
	       begin
