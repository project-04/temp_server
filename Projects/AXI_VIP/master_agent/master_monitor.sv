class master_monitor extends uvm_monitor;
	`uvm_component_utils(master_monitor)
      	...
      	virtual axi_if.MON_MP mif;
      	
      	axi_xtn			     xtn_write, xtn_read;
      	master_config 		     master_cfg;
      	
      	uvm_analysis_port #(axi_xtn) mst_monitor_port;
	...
	function void build_phase(uvm_phase phase);
		...
		if(! uvm_config_db #(master_config)::get(this,"","master_config",master_cfg))
        	...
        	mst_monitor_port = new("mst_monitor_port", this);
...
task master_monitor::collect_aw();
	...
	`uvm_info("MASTER_MONITOR", "Master Monitor starts collecting from Write Address Channel...", UVM_HIGH);
		...
		wait(mif.mon_cb.AWVALID && mif.mon_cb.AWREADY);
		
		xtn_write.AWVALID	= 1;
		xtn_write.AWREADY	= 1;
		xtn_write.AWADDR 	= mif.mon_cb.AWADDR;
		xtn_write.AWSIZE 	= mif.mon_cb.AWSIZE;	
	     	xtn_write.AWID 		= mif.mon_cb.AWID;
	     	xtn_write.AWLEN  	= mif.mon_cb.AWLEN;
	     	xtn_write.AWBURST 	= mif.mon_cb.AWBURST;
...
task master_monitor::collect_w(axi_xtn xtn_write);
	...
	`uvm_info("MASTER_MONITOR", "Master Monitor starts collecting from Write Data Channel...", UVM_HIGH);
		...
		foreach(xtn_write.WDATA[i])
			...
			begin
			     wait(mif.mon_cb.WVALID && mif.mon_cb.WREADY)
			     
			     xtn_write.WVALID	    = 1;
			     xtn_write.WREADY	    = 1;
			     xtn_write.WID	    = mif.mon_cb.WID;
			     
			     if(mif.mon_cb.WSTRB == 15)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[31:0];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 14)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[31:8];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 12)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[31:16];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 8)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[31:24];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;

			     if(mif.mon_cb.WSTRB == 4)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[23:16];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 3)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[15:0];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 2)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[15:8];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     	
			     if(mif.mon_cb.WSTRB == 1)
			     	xtn_write.WDATA[i]  = mif.mon_cb.WDATA[7:0];
			     	xtn_write.WSTRB[i]  = mif.mon_cb.WSTRB;
			     								
			     if(i == (xtn_write.AWLEN))
			        xtn_write.WLAST	= mif.mon_cb.WLAST;
...
task master_monitor::collect_b(axi_xtn xtn_write);
	...
	`uvm_info("MASTER_MONITOR", "Master Monitor starts collecting from Write Response Channel...", UVM_HIGH);
		...
		wait(mif.mon_cb.BVALID && mif.mon_cb.BREADY);
		xtn_write.BVALID	    = 1;
		xtn_write.BREADY	    = 1;
		xtn_write.BID		    = mif.mon_cb.BID;
		xtn_write.BRESP		    = mif.mon_cb.BRESP;
		
		@(mif.mon_cb);
		
		//$display($time," This is the master write transaction:%s",xtn_write.sprint());
		mst_monitor_port.write(xtn_write);
...
task master_monitor::collect_ar();
	...
	`uvm_info("MASTER_MONITOR", "Master Monitor starts collecting from Read Address Channel...", UVM_HIGH);
		...
		wait(mif.mon_cb.ARVALID && mif.mon_cb.ARREADY);
		
		xtn_read.ARVALID        = 1;
		xtn_read.ARREADY	= 1;
		xtn_read.ARID		= mif.mon_cb.ARID;
		xtn_read.ARLEN		= mif.mon_cb.ARLEN;
		xtn_read.ARSIZE		= mif.mon_cb.ARSIZE;
		xtn_read.ARBURST	= mif.mon_cb.ARBURST;	
		xtn_read.ARADDR		= mif.mon_cb.ARADDR;
	...
	`uvm_info("MASTER_MONITOR", "Master Monitor starts collecting from Read Data Channel...", UVM_HIGH);
		...
		xtn_read.RRESP		= new[xtn_read.ARLEN+1];
	 	...
	 	foreach(xtn_read.RDATA[i])
	 		begin
	 		     wait(mif.mon_cb.RVALID && mif.mon_cb.RREADY)
	 		     
	 		     xtn_read.RVALID    = 1;
			     xtn_read.RREADY	= 1;
			     xtn_read.RID	= mif.mon_cb.RID;
	 		     xtn_read.RDATA[i]	= mif.mon_cb.RDATA;
	 		     xtn_read.RRESP[i]	= mif.mon_cb.RRESP;
	 		     
	 		     if(i == (xtn_read.ARLEN))
	 		     	xtn_read.RLAST  = mif.mon_cb.RLAST;
		...
		mst_monitor_port.write(xtn_read);
