class slave_monitor extends uvm_monitor;
	`uvm_component_utils(slave_monitor)
      	...
      	virtual axi_if.MON_MP sif;
      	
      	axi_xtn			     xtn_write, xtn_read;
      	slave_config 		     slave_cfg;
      	
      	uvm_analysis_port #(axi_xtn) slv_monitor_port;
	...
	function void build_phase(uvm_phase phase);
		...
		if(! uvm_config_db #(slave_config)::get(this,"","slave_config",slave_cfg))
        	...
        	slv_monitor_port = new("slv_monitor_port", this);
...
task slave_monitor::collect_aw();
	...
	`uvm_info("SLAVE_MONITOR", "Slave Monitor starts collecting from Write Address Channel...", UVM_HIGH);
		...
		wait(sif.mon_cb.AWVALID && sif.mon_cb.AWREADY);
		
		xtn_write.AWVALID	= 1;
		xtn_write.AWREADY	= 1;
		xtn_write.AWADDR 	= sif.mon_cb.AWADDR;
		xtn_write.AWSIZE 	= sif.mon_cb.AWSIZE;	
	     	xtn_write.AWID 		= sif.mon_cb.AWID;
	     	xtn_write.AWLEN  	= sif.mon_cb.AWLEN;
	     	xtn_write.AWBURST 	= sif.mon_cb.AWBURST;
...
task slave_monitor::collect_w(axi_xtn xtn_write);
	...
	`uvm_info("SLAVE_MONITOR", "Slave Monitor starts collecting from Write Data Channel...", UVM_HIGH);
		...
		foreach(xtn_write.WDATA[i])
			...
			begin
			     wait(sif.mon_cb.WVALID && sif.mon_cb.WREADY)
			     
			     xtn_write.WVALID	    = 1;
			     xtn_write.WREADY	    = 1;
			     xtn_write.WID	    = sif.mon_cb.WID;
			     
			     if(sif.mon_cb.WSTRB == 15)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[31:0];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 14)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[31:8];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 12)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[31:16];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 8)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[31:24];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			
			     if(sif.mon_cb.WSTRB == 4)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[23:16];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 3)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[15:0];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 2)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[15:8];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     	
			     if(sif.mon_cb.WSTRB == 1)
			     	xtn_write.WDATA[i]  = sif.mon_cb.WDATA[7:0];
			     	xtn_write.WSTRB[i]  = sif.mon_cb.WSTRB;
			     								
			     if(i == (xtn_write.AWLEN))
			        xtn_write.WLAST	= sif.mon_cb.WLAST;
...
task slave_monitor::collect_b(axi_xtn xtn_write);
	...
	`uvm_info("SLAVE_MONITOR", "Slave Monitor starts collecting from Write Response Channel...", UVM_HIGH);
		...
		wait(sif.mon_cb.BVALID && sif.mon_cb.BREADY);
		xtn_write.BVALID	    = 1;
		xtn_write.BREADY	    = 1;
		xtn_write.BID		    = sif.mon_cb.BID;
		xtn_write.BRESP		    = sif.mon_cb.BRESP;
		
		@(sif.mon_cb);
		
		//$display($time," This is the slave write transaction:%s",xtn_write.sprint());
		slv_monitor_port.write(xtn_write);
...
task slave_monitor::collect_ar();
	...
	`uvm_info("SLAVE_MONITOR", "Slave Monitor starts collecting from Read Address Channel...", UVM_HIGH);
		...
		wait(sif.mon_cb.ARVALID && sif.mon_cb.ARREADY);
		
		xtn_read.ARVALID        = 1;
		xtn_read.ARREADY	= 1;
		xtn_read.ARID		= sif.mon_cb.ARID;
		xtn_read.ARLEN		= sif.mon_cb.ARLEN;
		xtn_read.ARSIZE		= sif.mon_cb.ARSIZE;
		xtn_read.ARBURST	= sif.mon_cb.ARBURST;	
		xtn_read.ARADDR		= sif.mon_cb.ARADDR;
	...
	`uvm_info("SLAVE_MONITOR", "Slave Monitor starts collecting from Read Data Channel...", UVM_HIGH)
		...
		xtn_read.RRESP		= new[xtn_read.ARLEN+1];
	 	...
	 	foreach(xtn_read.RDATA[i])
	 		begin
	 		     wait(sif.mon_cb.RVALID && sif.mon_cb.RREADY)
	 		     
	 		     xtn_read.RVALID    = 1;
			     xtn_read.RREADY	= 1;
			     xtn_read.RID	= sif.mon_cb.RID;
	 		     xtn_read.RDATA[i]	= sif.mon_cb.RDATA;
	 		     xtn_read.RRESP[i]	= sif.mon_cb.RRESP;
	 		     
	 		     if(i == (xtn_read.ARLEN))
	 		     	xtn_read.RLAST  = sif.mon_cb.RLAST;
		...
		slv_monitor_port.write(xtn_read);
