class env extends uvm_env;
	...
	function void connect_phase(uvm_phase phase);			
		foreach(master_agt_top.master_agt[i])
			master_agt_top.master_agt[i].master_monh.mst_monitor_port.connect(sb_h.mst_fifo[i].analysis_export);

		foreach(slave_agt_top.slave_agt[i])
			slave_agt_top.slave_agt[i].slave_monh.slv_monitor_port.connect(sb_h.slv_fifo[i].analysis_export);
	endfunction
