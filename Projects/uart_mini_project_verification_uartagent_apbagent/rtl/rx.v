module uart_rx(
   ...
   // DCS Dual Clock Sync or Dual Flop Cross Domain.
   always@(posedge PCLK)
   begin
   	temp_RXD <= RXD;
   	stable_RXD <= temp_RXD;
           ...
           case(rx_state)
             IDLE : 
               begin
                 ...
                 rx_buffer <= 0;
                 bit_counter <= 0;
                 if(stable_RXD==1'b0 && ~break_error)
                   begin
                     rx_state <= START;
                     rx_idle <= 0;
                   end
             ...
             START : 	
               begin
                 if(enable == 1) 
                   begin
		     if((stable_RXD==1'b0) && (bit_counter == 4'h8))     
                       begin
                         rx_state <= BIT0;
                         bit_counter <= 0;
                       end
	       ...
	       begin
                 if((enable == 1) && (bit_counter == 4'hf)) 
                   begin
                     rx_state <= STOP1;
                     case(LCR[5:3])
                       3'b001	: parity_bit <= ~(^({rx_buffer,stable_RXD})); // odd parity
                       3'b011	: parity_bit <= ^({rx_buffer,stable_RXD}); // even parity
                       3'b101	: parity_bit <= stable_RXD; // stick 1
                       3'b111	: parity_bit <= stable_RXD; // stick 0
      		 ...
      		 framing_error_temp <= ~stable_RXD;
                 if((enable == 1) && (bit_counter == 4'hf)) 
                   begin
                     rx_state <= STOP2;           
                     case(LCR[5:3])
                       3'b001	: parity_error <= (parity_bit != 0); // odd parity
                       3'b011	: parity_error <= (parity_bit != 0); // even parity
                       3'b101	: parity_error <= (parity_bit != 1); // Stick 1
                       3'b111	: parity_error <= (parity_bit != 0); // Stick 0
 ...
 // Works in conjuction with the receiver state machine
   ...
   always @(posedge PCLK) 
     begin
       if(PRESETn==1'b0)
         counter_b <= brc_value-8'd8; //8'd159; //-8 because we are sampling at middle 8 buad pulse, so 16-8 = 8, so -8 from the brcvalue
       else if (stable_RXD)
         counter_b <= brc_value-8'd8; // character time length - 1 //-8 because we are sampling at middle 8 buad pulse, so 16-8 = 8, so -8 from the brcvalue
   ...
   always @(posedge PCLK)
     begin
       if(PRESETn==1'b0)
         counter_t <= toc_value-8'd8; //10'd639; // 10 bits for the default 8N1 //-8 because we are sampling at middle 8 buad pulse, so 16-8 = 8, so -8 from the brcvalue
       else 
	 if(push_rx_fifo || pop_rx_fifo || rx_fifo_count == 0) // counter is reset when RX FIFO is empty, accessed or above trigger level
           counter_t <= toc_value-8'd8; //-8 because we are sampling at middle 8 buad pulse, so 16-8 = 8, so -8 from the brcvalue
         else 
